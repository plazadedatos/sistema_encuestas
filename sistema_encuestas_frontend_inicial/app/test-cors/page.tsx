...
export default function TestCorsPage() {
  ...
  const testPublicEndpoint = async () => {
    ...
    try {
      ...
      setResult(
        `✅ CORS funciona correctamente: ${JSON.stringify(response.data, null, 2)}`
      );
  ...
  const testProtectedEndpoint = async () => {
    ...
    try {
      ...
      setResult(
        `✅ Endpoint protegido funciona: ${JSON.stringify(response.data, null, 2)}`
      );
      console.log('✅ Respuesta del endpoint protegido:', response.data);
    } catch (error: any) {
      setResult(
        `❌ Error en endpoint protegido: ${error.message} (Status: ${error.response?.status})`
      );
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">
          Pruebas de CORS y Autenticación
        </h1>
        ...
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <ul className="list-disc list-inside text-yellow-700 space-y-1">
            <li>
              <strong>Endpoint Público:</strong> Debe funcionar sin token
              (prueba CORS)
            </li>
            <li>
              <strong>Endpoint Protegido:</strong> Requiere estar logueado como
              administrador
            </li>
            <li>
              <strong>Info de Token:</strong> Muestra si hay datos de sesión
              guardados
            </li>
          </ul>
        </div>
